...
function get_same_level_html ( $t ) {
  ...
  for ( $pos = 0 ; $pos < strlen ( $t ) ; $pos++ ) {
    if ( $t[$pos] == '<' && $t[$pos+1] == '/' && $cnt == 0 ) return trim ( substr ( $t , 0 , $pos ) ) ;
...
function try_fileinfo_template ( $text ) {
  global $titles ;
  $matches = array () ;
  preg_match_all ( '/t[dh][^>]+id="fileinfotpl_[^"]+"/' , $text , $matches ) ;
  $matches = $matches[0] ;
...
<hr/>
Usage : <b>original.php?image=IMAGENAME</b><br/>
...
Examples :
<ul>
<li><a href='original.php?image=Sa-warthog.jpg&thumbwidth=150&thumbheight=150&versions&meta'>Sa-warthog.jpg</a> with thumbnail, version, and meta information</li>
<li><a href='original.php?image=Yarra_Panorama.jpg&forcehtml'>Yarra_Panorama.jpg</a> (contains geocoding, which resolves to a <tt>&lt;location&gt;</tt> tag)</li>
<li><a href='original.php?image=Sa-warthog.jpg|Yarra_Panorama.jpg|filedoesnotexist&thumbwidth=150&thumbheight=150&versions&meta'>Multiple images</a> and one (deliberate) error, plus meta information</li>
</ul>
...
foreach ( $imagelist AS $img ) {
	...
	$filedata = array () ;
	if ( !isset ( $data ) ) {
	  end_run ( 'File does not exist : ' . $img ) ;
	  continue ;
	}
	foreach ( $data AS $k => $v ) {
	  $fd = $v ;
